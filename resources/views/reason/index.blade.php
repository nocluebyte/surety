...
@section('content')
    @component('partials._subheader.subheader-v6', [
        'page_title' => $title,
        'add_modal' => collect([
            'action' => route('reason.create'),
            'target' => '#commonModalID',
            'text' => __('common.add'),
        ]),
        'back_text' => __('common.back'),
        'model_back_action' => route('masterPages'),
        'permission' => $current_user->hasAnyAccess(['reason.add', 'users.superadmin']),
    ])
    @endcomponent
...
@section('scripts')
    <script type="text/javascript">
        let reason = "{{ __('reason.reason') }}";
        let description = "{{ __('reason.description') }}";
