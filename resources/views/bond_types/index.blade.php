@extends($theme)
@section('title', __('bond_types.bond_types'))

@section('content')
    @component('partials._subheader.subheader-v6', [
        'page_title' => __('bond_types.bond_types'),
        'add_modal' => collect([
            'action' => route('bond_types.create'),
            'target' => '#commonModalID',
            'text' => __('common.add'),
        ]),
        'back_text' => __('common.back'),
        'model_back_action' => route('masterPages'),
        'permission' => $current_user->hasAnyAccess(['bond_types.add', 'users.superadmin']),
    ])
    @endcomponent
                    ...
                    <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <div class="datatable-form-filter no-padding">
                                        {!! Form::text('name', null, ['class' => 'form-control bname']) !!}
                                    </div>
                                </th>
                                <th>
                                    <div class="datatable-form-filter no-padding">
                            ...
                            <tr>
                                ...
                                <th class="d-none"></th>
                                <th>{{ __('bond_types.name') }}</th>
                                <th>{{ __('bond_types.prefix') }}</th>
...
@section('scripts')
    <script type="text/javascript">
        let name = "{{ __('bond_types.name') }}";
        let prefix = "{{ __('bond_types.prefix') }}";
            ...
            window.LaravelDataTables['dataTableBuilder'] = $('#dataTableBuilder').DataTable({
                ...
                "ajax": {
                    data: function(d) {
                        ...
                        d.prefix = jQuery(".datatable-form-filter input[name='prefix']").val();
                    }
                },
