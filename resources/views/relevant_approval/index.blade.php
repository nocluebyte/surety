@extends($theme)
@section('title', __('relevant_approval.relevant_approval'))

@section('content')
    @component('partials._subheader.subheader-v6', [
        'page_title' => __('relevant_approval.relevant_approval'),
        'add_modal' => collect([
            'action' => route('relevant_approval.create'),
            'target' => '#commonModalID',
            'text' => __('common.add'),
        ]),
        'back_text' => __('common.back'),
        'model_back_action' => route('masterPages'),
        'permission' => $current_user->hasAnyAccess(['relevant_approval.add', 'users.superadmin']),
    ])
    @endcomponent
                    ...
                    <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th class="d-none"></th>
                                <th>{{ __('relevant_approval.name') }}</th>
...
@section('scripts')
    <script type="text/javascript">
        let name = "{{ __('relevant_approval.name') }}";
