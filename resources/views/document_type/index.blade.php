@extends($theme)
@section('title', __('document_type.document_type'))

@section('content')
    @component('partials._subheader.subheader-v6', [
        'page_title' => __('document_type.document_type'),
        'add_modal' => collect([
            'action' => route('document_type.create'),
            'target' => '#commonModalID',
            'text' => __('common.add'),
        ]),
        'back_text' => __('common.back'),
        'model_back_action' => route('masterPages'),
        'permission' => $current_user->hasAnyAccess(['document_type.add', 'users.superadmin']),
    ])
    @endcomponent
                    ...
                    <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th class="d-none"></th>
                                <th>{{ __('document_type.name') }}</th>
                                <th>{{ __('document_type.type') }}</th>
...
@section('scripts')
    <script type="text/javascript">
        let name = "{{ __('document_type.name') }}";
        let type = "{{ __('document_type.type') }}";
