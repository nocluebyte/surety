<ul class="nav nav-tabs nav-light-info nav-bold  ">
    <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#invocation_general">
            <span class="nav-text">{{ __('invocation_notification.general') }}</span>
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-toggle="tab" href="#invocation_review">
            <span class="nav-text">{{ __('cases.review') }}</span>
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-toggle="tab" href="#invocation_other">
            <span class="nav-text">{{ __('cases.other') }}</span>
        </a>
    </li>
</ul>
{{-- @dd($dms_data->except('attachment')) --}}
{{-- @dd($dms_data['attachment'])
@dd(!in_array('attachment', $dms_data)) --}}
{{-- @dd($dms_data) --}}
<div class="tab-content mt-5">
    <div class="tab-pane fade show active" id="invocation_general">
        <div class="row">
            <div class="col-lg-12 overflow-auto" style="height: 170px;">
                <table class="table table-separate table-head-custom table-checkable">
                    ...
                    <tbody>
                        @forelse ($general_docs ?? [] as $dms_key => $dms_attachment)
                            <tr>
                                ...
                                <td><a data-toggle="modal" data-target-modal="#commonModalID"
                                        data-url="{{ route('invocationNotification.dmsattachmentdownload', $dms_attachment->id) }}"
                                        ...
                                        <ul class="navi navi-hover ">
                                            ...
                                            <li class="navi-item">
                                                <a href="#" data-toggle="modal" data-target-modal="#commonModalID"
                                                    data-url="{{ route('invocationNotification.dmsAttachmentcomment.get', $dms_attachment->id) }}"
                                                    class="call-modal navi-link">
                                            ...
                                            <li class="navi-item">
                                                <a href="#" data-toggle="modal" data-target-modal="#dms_attachment_modal"
                                                    data-url="{{ route('invocationNotification.dmsAttachmentCommentLog', $dms_attachment->id) }}"
                                                    class="call-modal navi-link">
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        {!! Form::model($dms_attachment, [
                                            'route' => ['invocationNotification.dmsupdate', $dms_attachment->id],
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                {{ Form::select('document_type_id', ['' => 'Select'] + $document_type, $dms_attachment->document_type_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required', 'data-placeholder' => 'Select Document Type', 'id' => 'document_type_amend_general' . $dms_key]) }}
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('file_source_id', __('cases.file_source')) }}
                                                {{ Form::select('file_source_id', ['' => 'Select'] + $file_source, $dms_attachment->file_source_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required', 'data-placeholder' => 'Select File Source', 'id' => 'file_source_amend_general' . $dms_key]) }}
    ...
    <div class="tab-pane fade show" id="invocation_review" role="tabpanel">
        <div class="row">
            <div class="col-lg-12 overflow-auto" style="height: 170px;">
                <table class="table table-separate table-head-custom table-checkable">
                    <thead>
                    ...
                    <tbody>
                        @forelse ($review_docs ?? [] as $dms_key => $dms_attachment)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $dms_attachment->documentType->name ?? '' }}</td>
                                <td>{{ $dms_attachment->createdBy->full_name ?? '' }}</td>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                {{ Form::label('document_type_id', __('cases.document_type')) }}
                                                {{ Form::select('document_type_id', ['' => 'Select'] + $document_type, $dms_attachment->document_type_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required', 'data-placeholder' => 'Select Document Type', 'id' => 'document_type_amend_review' . $dms_key]) }}
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('file_source_id', __('cases.file_source')) }}
                                                {{ Form::select('file_source_id', ['' => 'Select'] + $file_source, $dms_attachment->file_source_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required', 'data-placeholder' => 'Select File Source', 'id' => 'file_source_amend_review' . $dms_key]) }}
    ...
    <div class="tab-pane fade show" id="invocation_other" role="tabpanel">
        <div class="row">
            <div class="col-lg-12 overflow-auto" style="height: 170px;">
                <table class="table table-separate table-head-custom table-checkable">
                    <thead>
                    ...
                    <tbody>
                        @forelse ($other_docs ?? [] as $dms_key => $dms_attachment)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $dms_attachment->documentType->name ?? '' }}</td>
                                <td>{{ $dms_attachment->createdBy->full_name ?? '' }}</td>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                {{ Form::label('document_type_id', __('cases.document_type')) }}
                                                {{ Form::select('document_type_id', ['' => 'Select'] + $document_type, $dms_attachment->document_type_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required', 'data-placeholder' => 'Select Document Type', 'id' => 'document_type_amend_other' . $dms_key]) }}
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('file_source_id', __('cases.file_source')) }}
                                                {{ Form::select('file_source_id', ['' => 'Select'] + $file_source, $dms_attachment->file_source_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required', 'data-placeholder' => 'Select File Source', 'id' => 'file_source_amend_other' . $dms_key]) }}
...
<div class="modal fade" id="dmsModal" tabindex="-1" role="dialog" aria-labelledby="dmsModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            {!! Form::open([
                'route' => 'invocationNotification.dmsattachment',
                'id' => 'dmsTabForm',
            ...
            <div class="modal-body">
                {!! Form::hidden('id', $invocationData->id) !!}
                <div class="form-group">
                    {!! Form::label('final_submission', __('cases.final_submission')) !!}<span class="text-danger">*</span>
                    <div class="radio-inline pt-4">
                        <label class="radio radio-rounded">
                ...
                <div class="form-group">
                    ...
                    {!! Form::select('document_type_id', ['' => ''] + $document_type, null, [
                        'class' => 'form-control document_type jsSelect2ClearAllow',
                        'required' => true,
                ...
                <div class="form-group">
                    ...
                    {!! Form::select('file_source_id', ['' => ''] + $file_source, null, [
                        'class' => 'form-control file_source jsSelect2ClearAllow',
                        'required' => true,
                        'data-placeholder'=>'Select'
                    ]) !!}
                </div>
                <div class="form-group">
                    ...
                    {!! Form::file('attachment[]', ['class' => 'form-control', 'required' => true, 'multiple' => true]) !!}
                </div>
            </div>
            <div class="modal-footer">
