@extends($theme)
@section('title', __('financing_sources.financing_sources'))

@section('content')
    @component('partials._subheader.subheader-v6', [
        'page_title' => __('financing_sources.financing_sources'),
        'add_modal' => collect([
            'action' => route('financing_sources.create'),
            'target' => '#commonModalID',
            'text' => __('common.add'),
        ]),
        'back_text' => __('common.back'),
        'model_back_action' => route('masterPages'),
        'permission' => $current_user->hasAnyAccess(['financing_sources.add', 'users.superadmin']),
    ])
    @endcomponent
                    ...
                    <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th class="d-none"></th>
                                <th>{{ __('financing_sources.name') }}</th>
...
@section('scripts')
    <script type="text/javascript">
        let name = "{{ __('financing_sources.name') }}";
