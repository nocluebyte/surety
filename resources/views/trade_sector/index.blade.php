@extends($theme)
@section('title', __('trade_sector.trade_sector'))

@section('content')
    @component('partials._subheader.subheader-v6', [
        'page_title' => __('trade_sector.trade_sector'),
        'add_modal' => collect([
            'action' => route('trade_sector.create'),
            'target' => '#commonModalID',
            'text' => __('common.add'),
        ]),
        'back_text' => __('common.back'),
        'model_back_action' => route('masterPages'),
        'permission' => $current_user->hasAnyAccess(['trade_sector.add', 'users.superadmin']),
    ])
    @endcomponent
                    ...
                    <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <div class="datatable-form-filter no-padding">
                                        {!! Form::text('mid_level', null, ['class' => 'form-control']) !!}
                                    </div>
                                </th>
                                <th></th>
                            </tr>
                            ...
                            <tr>
                                ...
                                <th class="d-none"></th>
                                <th>{{ __('trade_sector.name') }}</th>
                                <th>{{ __('trade_sector.mid_level') }}</th>
...
@section('scripts')
    <script type="text/javascript">
        let name = "{{ __('trade_sector.name') }}";
        let mid_level = "{{ __('trade_sector.mid_level') }}";
