                            ...
                            <table>
                                <tr>
                                    ...
                                    <td class="w-225px filter_{{$dval}}">
                                        {!! Form::number('balancea['.$balanceakey.']['.$dval.']', $balanceSheetData[$balanceakey][$dval] ?? null, ['class' => 'form-control jsBalancea '.$soild.' '.$balanceakey.'_'.$dval .' '.$dnone.' '.$class_a,'id'=>'balancea['.$balanceakey.']['.$dval.']','data-slug' => $balanceakey,'data-year' => $dval,$readonly,$disable,'']) !!}
                                    </td>
                                    @endforeach
                                    @endif
                                </tr>
                    ...
                    <table>
                        <tr>
                            @if(isset($datesArr) && count($datesArr)>0)
                                @foreach($datesArr as $dkey => $dval)
                                    <td class="w-225px filter_{{$dval}}">
                                        {!! Form::number('balanceb['.$balancebkey.']['.$dval.']', $balanceSheetData[$balancebkey][$dval] ?? null, ['class' => 'form-control jsBalanceb '.$soild.' '.$balancebkey.'_'.$dval.' '.$dnone.' '.$class_b.' '. $class,'id'=>'balanceb['.$balancebkey.']['.$dval.']','data-slug' => $balancebkey,'data-year' => $dval, $readonly,$disable,'']) !!}
                                    </td>
                                @endforeach
                            @endif
                        </tr>
