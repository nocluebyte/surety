...
@component('partials._subheader.subheader-v6', [
    ...
    'add_modal' => collect([
        'action' => route('dms.create', ['type' => encryptId('invocation_notification'), 'contractor_id' => encryptId($contractor_id), 'invocation_notification_id' => encryptId($invocation_notification_id)]),
        'target' => '#commonModalID',
        'text' => __('common.add'),
    ]),
    'text' => __('common.back'),
        ...
        <div class="col-sm-12">
            <div class="row g-6 g-xl-9 mb-6 mb-xl-9">
                @if ($isfilter && $dms->isNotEmpty())
                    @foreach ($dms->groupBy('attachment_type') as $key => $docs)
                        <div class="col-12 my-5">
                            <h5><strong>{{ $docs->first()->proposalCode }}</strong></h5>
                            <h6>{{ Str::headline($key) }}</h6>
                        </div>
