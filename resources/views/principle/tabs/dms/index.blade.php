<ul class="nav nav-tabs nav-light-info nav-bold  ">
    <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#dms_review">
            <span class="nav-text">{{ __('cases.review') }}</span>
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-toggle="tab" href="#dms_application">
            <span class="nav-text">{{ __('cases.application') }}</span>
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-toggle="tab" href="#dms_other">
            <span class="nav-text">{{ __('cases.other') }}</span>
        </a>
    </li>
</ul>
<div class="tab-content mt-5">
    <div class="tab-pane fade show active" id="dms_review" role="tabpanel">
        <div class="col-lg-12 overflow-auto" style="height: 170px;">
            <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                ...
                <tbody>
                    @forelse ($reviewDocs as $dms_key => $dms_attachment)
                        <tr>
                            ...
                            <td><a data-toggle="modal" data-target-modal="#commonModalID"
                                    data-url="{{ route('principleDmsattachmentdownload', $dms_attachment->id) }}"
                                        ...
                                        <ul class="navi navi-hover ">
                                            ...
                                            <li class="navi-item">
                                                <a href="#" data-toggle="modal" data-target-modal="#commonModalID"
                                                    data-url="{{ route('principleDmsAttachmentComment.get', $dms_attachment->id) }}"
                                                    class="call-modal navi-link">
                                            ...
                                            <li class="navi-item">
                                                <a href="#" data-toggle="modal" data-target-modal="#commonModalID"
                                                    data-url="{{ route('principleDmsAttachmentCommentLog', $dms_attachment->id) }}"
                                                    class="call-modal navi-link">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    {!! Form::model($dms_attachment, [
                                        'route' => ['principle-dms-update', $dms_attachment->id],
                                        'id' => 'amendmentForm_' . $dms_attachment->id,
                                    ]) !!}
                                    @method('PUT')
                                    <input type="hidden" name="cases_id" value="{{ $dms_attachment->caseId ?? '' }}">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            {{ Form::select('document_type_id', ['' => 'Select'] + $document_type, $dms_attachment->document_type_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required document_type', 'data-placeholder' => 'Select Document Type', 'id' => 'document_type_amend_review' . $dms_key]) }}
                                        </div>
                                        <div class="form-group">
    ...
    <div class="tab-pane fade show" id="dms_application" role="tabpanel">
        <div class="col-lg-12 overflow-auto" style="height: 370px;">
            @foreach($applicationDocs as $key => $item)
                <div>
                    <h5>{{ $key }}</h5>
                </div>
                <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                    ...
                    <tbody>
                        @forelse ($item as $dms_key => $dms_attachment)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $dms_attachment->documentType->name ?? '' }}</td>
                                <td>{{ $dms_attachment->createdBy->full_name ?? '' }}</td>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                {{ Form::label('document_type_id', __('cases.document_type')) }}
                                                {{ Form::select('document_type_id', ['' => 'Select'] + $document_type, $dms_attachment->document_type_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required document_type', 'data-placeholder' => 'Select Document Type', 'id' => 'document_type_amend_application' . $dms_key . $key]) }}
                                            </div>
                                            <div class="form-group">
                                                {{ Form::label('file_source_id', __('cases.file_source')) }}
                                                {{ Form::select('file_source_id', ['' => 'Select'] + $file_source, $dms_attachment->file_source_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required', 'data-placeholder' => 'Select File Source', 'id' => 'file_source_amend_application' . $dms_key . $key]) }}
    ...
    <div class="tab-pane fade show" id="dms_other" role="tabpanel">
        <div class="col-lg-12 overflow-auto" style="height: 170px;">
            <table class="table table-separate table-head-custom table-checkable" id="dataTableBuilder">
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse ($otherDocs as $dms_key => $dms_attachment)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $dms_attachment->documentType->name ?? '' }}</td>
                            <td>{{ $dms_attachment->createdBy->full_name ?? '' }}</td>
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            {{ Form::label('document_type_id', __('cases.document_type')) }}
                                            {{ Form::select('document_type_id', ['' => 'Select'] + $document_type, $dms_attachment->document_type_id ?? '', ['class' => 'form-select jsSelect2ClearAllow required document_type', 'data-placeholder' => 'Select Document Type', 'id' => 'document_type_amend_other' . $dms_key]) }}
                                        </div>
                                        <div class="form-group">
                                            {{ Form::label('file_source_id', __('cases.file_source')) }}
