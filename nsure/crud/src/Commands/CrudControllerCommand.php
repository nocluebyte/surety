...
class CrudControllerCommand extends GeneratorCommand
{
    ...
    protected function getStub()
    {
        ...
        return __DIR__ . '/../stubs/' . config('crudgenerator.default') . '/controller.stub';
    }
    ...
    protected function buildClass($name)
    {
        ...
        $datatableName = $this->option('datatable-name');
        ...
        return $this->replaceNamespace($stub, $name)
            ...
            ->replaceDatatableName($stub, $datatableName)
            ->replaceRouteGroup($stub, $routeGroup)
            ->replaceValidationRules($stub, $validationRules)
            ->replaceGlobalSearch($stub,$this->option('fields'))
    ...
    protected function replaceDatatableName(&$stub, $datatableName)
    {
        $stub = str_replace(
            '{{datatableName}}', $datatableName, $stub
        );
    ...
    public function replaceGlobalSearch(&$stub,$fields)
    {
        if($this->option('stubtype') == 'vue')
        {
            ...
            foreach ($fieldsArray as $item) {
                ...
                $str.='->orWhere("'.trim($itemArray[0]).'","LIKE","%".$filter."%")';
            }
            $str.=';';
            $stub = str_replace(
                '{{global_search}}', $str, $stub
            );
