...
class InsuranceCompaniesDataTable extends DataTable
{
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('insurance_companies.edit', [encryptId($row->id)]);
        $deleteUrl = route('insurance_companies.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'insurance_companies.edit', 'insurance_companies.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['insurance_companies.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['insurance_companies.delete', 'users.superadmin']))
        {
    ...
    {
        $model = InsuranceCompanies::select('insurance_companies.*');
        ...
        if(request()->get('phone_no')) {
            $model->where('phone_no', 'like', '%' . request()->get('phone_no') . '%');
        }
    ...
    {
        return [
            ['name' => 'insurance_companies', 'data' => 'insurance_companies', 'title' => trans("insurance_companies.insurance_companies")],
    ...
    // {
    //     return 'InsuranceCompanies_' . date('YmdHis');
    // }
}
