...
class RelevantApprovalDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('relevant_approval.edit', [encryptId($row->id)]);
        $deleteUrl = route('relevant_approval.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'relevant_approval.edit', 'relevant_approval.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['relevant_approval.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['relevant_approval.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(RelevantApproval $model)
    {
        $model = RelevantApproval::select('relevant_approvals.*');
    ...
    {
        return [
            ['name' => 'relevant_approval', 'data' => 'relevant_approval', 'title' => trans("relevant_approval.relevant_approval")],
    ...
    // {
    //     return 'RelevantApproval_' . date('YmdHis');
    // }
}
