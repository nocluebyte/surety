...
class HsnCodeDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('hsn-code.edit', [encryptId($row->id)]);
        $deleteUrl = route('hsn-code.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'hsn-code.edit', 'hsn-code.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['hsn-code.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['hsn-code.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(HsnCode $model)
    {
        $model = HsnCode::select('hsn_code', 'gst', 'is_service','description', 'id', 'is_active');

        if(request()->get('hsn_code')){
            $model->where('hsn_code', 'like', '%' . request()->get('hsn_code') . '%');
        }

        if(request()->get('gst')){
            $model->where('gst', 'like', '%' . request()->get('gst') . '%');
        }

        if(request()->get('is_service')){
            $model->where('is_service', 'like', '%' . request()->get('is_service') . '%');
        }
