...
class BankingLimitCategoryDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('banking_limit_categories.edit', [encryptId($row->id)]);
        $deleteUrl = route('banking_limit_categories.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'banking_limit_categories.edit', 'banking_limit_categories.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['banking_limit_categories.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['banking_limit_categories.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(BankingLimitCategory $model)
    {
        $model = BankingLimitCategory::select('banking_limit_categories.*');
    ...
    {
        return [
            ['name' => 'banking_limit_categories', 'data' => 'banking_limit_categories', 'title' => trans("banking_limit_categories.banking_limit_categories")],
    ...
    // {
    //     return 'BankingLimitCategory_' . date('YmdHis');
    // }
}
