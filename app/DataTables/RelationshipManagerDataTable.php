...
class RelationshipManagerDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('relationship_manager.edit', [encryptId($row->id)]);
        $deleteUrl = route('relationship_manager.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'relationship_manager.edit', 'relationship_manager.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['relationship_manager.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['relationship_manager.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(RelationshipManager $model)
    {
        $field = [
            DB::raw("CONCAT_WS(' ', users.first_name,users.middle_name,users.last_name) as full_name"),
            "users.email",
            "users.mobile as phone_no",
            "relationship_managers.id",
            "relationship_managers.city",
            "relationship_managers.is_active",
        ];
        $model = RelationshipManager::select($field)->leftJoin('users' ,function ($join) {
            $join->on('relationship_managers.user_id', '=', 'users.id');
        ...
        if (request()->get('full_name',false)) {
            $model->whereHas('user',function($q) use($fullName){
                $q->whereRaw("CONCAT_WS(' ',first_name,middle_name,last_name) LIKE ?", '%' . $fullName . '%');
            });
        }
    ...
    {
        return $this->builder()
                    ->setTableId('relationshipmanager-table')
    ...
    // {
    //     return 'RelationshipManager_' . date('YmdHis');
    // }
}
