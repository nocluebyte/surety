...
class BondTypesDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('bond_types.edit', [encryptId($row->id)]);
        $deleteUrl = route('bond_types.destroy', [encryptId($row->id)]);

        if($user->hasAnyAccess(['users.info', 'bond_types.edit', 'bond_types.delete', 'users.superadmin']))
        {
        ...
        if($user->hasAnyAccess(['bond_types.edit', 'users.superadmin']))
        {
        ...
        if($user->hasAnyAccess(['bond_types.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(BondTypes $models)
    {
        $models = BondTypes::select();
        ...
        if(request()->get('prefix')){
            $models->where('prefix', 'like', '%' . request()->get('prefix') . '%');
        }
    ...
    {
        return [
            ['name' => 'bond_types', 'data' => 'bond_types', 'title' => trans("bond_types.bond_types")],
