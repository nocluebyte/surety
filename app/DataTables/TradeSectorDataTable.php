...
class TradeSectorDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('trade_sector.edit', [encryptId($row->id)]);
        $deleteUrl = route('trade_sector.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'trade_sector.edit', 'trade_sector.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['trade_sector.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['trade_sector.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(TradeSector $model)
    {
        $model = TradeSector::select('trade_sectors.id', 'trade_sectors.name', 'sector_ratings.name as mid_level', 'trade_sectors.is_active')
        ->leftJoin('sector_ratings', function($join){
            $join->on('trade_sectors.mid_level', '=', 'sector_ratings.id');
        })
        ->whereNull('trade_sectors.deleted_at');

        if(request()->get('name')){
            $model->where('trade_sectors.name', 'like', '%' . request()->get('name') . '%');
        }

        if(request()->get('mid_level')){
            $model->where('sector_ratings.name', 'like', '%' . request()->get('mid_level') . '%');
        }
    ...
    {
        return [
            ['name' => 'trade_sector', 'data' => 'trade_sector', 'title' => trans("trade_sector.trade_sector")],
    ...
    // {
    //     return 'TradeSector_' . date('YmdHis');
    // }
}
