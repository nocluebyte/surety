...
class FacilityTypeDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('facility_type.edit', [encryptId($row->id)]);
        $deleteUrl = route('facility_type.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'facility_type.edit', 'facility_type.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['facility_type.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['facility_type.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(FacilityType $model)
    {
        $model = FacilityType::select('facility_types.*');
    ...
    {
        return [
            ['name' => 'facility_type', 'data' => 'facility_type', 'title' => trans("facility_type.facility_type")],
    ...
    // {
    //     return 'FacilityType_' . date('YmdHis');
    // }
}
