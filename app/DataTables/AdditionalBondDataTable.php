...
class AdditionalBondDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('additional-bond.edit', [encryptId($row->id)]);
        $deleteUrl = route('additional-bond.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'additional_bond.edit', 'additional_bond.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['additional_bond.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['additional_bond.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(AdditionalBond $model)
    {
        $model = AdditionalBond::select('name', 'id', 'is_active');
    ...
    {
        return [
            ['name' => 'additional_bond', 'data' => 'additional_bond', 'title' => trans("additional_bond.additional_bond")],
    ...
    {
        return 'AdditionalBond_' . date('YmdHis');
    }
}
