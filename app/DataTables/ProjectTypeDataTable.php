...
class ProjectTypeDataTable extends DataTable
{
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('project_type.edit', [encryptId($row->id)]);
        $deleteUrl = route('project_type.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'project_type.edit', 'project_type.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['project_type.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['project_type.delete', 'users.superadmin']))
        {
    ...
    {
        $model = ProjectType::select('project_types.*');
    ...
    {
        return [
            ['name' => 'project_type', 'data' => 'project_type', 'title' => trans("project_type.project_type")],
