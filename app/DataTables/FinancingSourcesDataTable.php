...
class FinancingSourcesDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('financing_sources.edit', [encryptId($row->id)]);
        $deleteUrl = route('financing_sources.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'financing_sources.edit', 'financing_sources.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['financing_sources.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['financing_sources.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(FinancingSources $models)
    {
        $models = FinancingSources::select('id', 'name', 'is_active');
    ...
    {
        return [
            ['name' => 'financing_sources', 'data' => 'financing_sources', 'title' => trans("financing_sources.financing_sources")],
    ...
    // {
    //     return 'FinancingSources_' . date('YmdHis');
    // }
}
