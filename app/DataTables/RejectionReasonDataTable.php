...
class RejectionReasonDataTable extends DataTable
{
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('rejection-reason.edit', [encryptId($row->id)]);
        $deleteUrl = route('rejection-reason.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'rejection_reason.edit', 'rejection_reason.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['rejection_reason.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['rejection_reason.delete', 'users.superadmin']))
        {
    ...
    {
        $model = RejectionReason::select('reason', 'description', 'id', 'is_active');

        if(request()->get('reason')){
            $model->where('reason', 'like', '%' . request()->get('reason') . '%');
        }
    ...
    {
        return $this->builder()
                    ->setTableId('rejectionreason-table')
