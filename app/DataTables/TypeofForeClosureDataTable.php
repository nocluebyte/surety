...
class TypeofForeClosureDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('type-of-foreclosure.edit', [encryptId($row->id)]);
        $deleteUrl = route('type-of-foreclosure.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'type_of_foreclosure.edit', 'type_of_foreclosure.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['type_of_foreclosure.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['type_of_foreclosure.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(TypeofForeClosure $model)
    {
        $model = TypeofForeClosure::select('id', 'name', 'is_active');
    ...
    {
        return $this->builder()
                    ->setTableId('typeofforeclosure-table')
    ...
    {
        return 'TypeofForeClosure_' . date('YmdHis');
    }
}
