...
class FileSourceDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('file_source.edit', [encryptId($row->id)]);
        $deleteUrl = route('file_source.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'file_source.edit', 'file_source.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['file_source.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['file_source.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(FileSource $model)
    {
        $model = FileSource::select('file_sources.*');
    ...
    {
        return [
            ['name' => 'file_source', 'data' => 'file_source', 'title' => trans("file_source.file_source")],
    ...
    // {
    //     return 'FileSource_' . date('YmdHis');
    // }
}
