...
class DocumentTypeDataTable extends DataTable
{
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('document_type.edit', [encryptId($row->id)]);
        $deleteUrl = route('document_type.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'document_type.edit', 'document_type.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['document_type.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['document_type.delete', 'users.superadmin']))
        {
    ...
    {
        $model = DocumentType::select('document_types.*');
    ...
    {
        return [
            ['name' => 'document_type', 'data' => 'document_type', 'title' => trans("document_type.document_type")],
