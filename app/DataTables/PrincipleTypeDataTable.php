...
class PrincipleTypeDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('principle_type.edit', [encryptId($row->id)]);
        $deleteUrl = route('principle_type.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'principle_type.edit', 'principle_type.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['principle_type.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['principle_type.delete', 'users.superadmin']))
        {
     ...
     */
    public function query(PrincipleType $model)
    {
        $model = PrincipleType::select('principle_types.*');
    ...
    {
        return [
            ['name' => 'principle_type', 'data' => 'principle_type', 'title' => trans("principle_type.principle_type")],
    ...
    // {
    //     return 'PrincipleType_' . date('YmdHis');
    // }
}
