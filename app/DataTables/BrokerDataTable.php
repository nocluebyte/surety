...
class BrokerDataTable extends DataTable
{
    ...
    public function checkrights($row)
    {
        ...
        $menu = '';
        $editUrl = route('broker.edit', [encryptId($row->id)]);
        $deleteUrl = route('broker.destroy', [encryptId($row->id)]);

        if($user -> hasAnyAccess(['users.info', 'broker.edit', 'broker.delete', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['broker.edit', 'users.superadmin']))
        {
        ...
        if($user -> hasAnyAccess(['broker.delete', 'users.superadmin']))
        {
    ...
    public function query(Broker $model)
    {
        ...
        $model = Broker::select($field)->leftJoin('users',function($join){
            $join->on('brokers.user_id','=','users.id');
    ...
    {
        return [
            ['name' => 'broker', 'data' => 'broker', 'title' => trans("broker.broker")],
